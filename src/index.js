  ...
  _createClass(Sladio, [
    ...
    },
    {
      ...
      value: function createDefaultSlider(slider) {
        ...
        var pagActive =
          this._config.pagination[slider.getAttribute('id')].pagActive;

        var autoSlideActive =
          this._config.autoSlide[slider.getAttribute('id')].autoSlideActive; // Si la orientación es vertical
        ...
        if (autoSlideActive) {
          var _this$_config$autoSli =
              this._config.autoSlide[slider.getAttribute('id')],
    ...
    },
    {
      ...
      value: function createNavsButtons(slider, navsButtons) {
        ...
        if (
          !slider.querySelector('.'.concat(btnPrev)) &&
          !slider.querySelector('.'.concat(btnNext))
        ) {
    ...
    },
    {
      ...
      value: function nextSlide(slider, container, items) {
        ...
        if (pagActive) {
          ...
          if (
            !slider
              .querySelector('.sladio__indicator__bullets a:last-child')
              .classList.contains('bullet_selected')
          ) {
            container.scrollLeft += slider.scrollWidth; // Muestra el siguiente item
          } else {
            slider
              .querySelector('.sladio__indicator__bullets a:first-child')
              .click();
    ...
    },
    {
      ...
      value: function createIndicators(slider, orientation) {
        ...
        var _this$_config$paginat =
            this._config.pagination[slider.getAttribute('id')],
        ...
        if (type !== 'bullets') {
          slider.querySelector('.sladio__indicator__bullets').style.display =
            'none';
        }
      }, // Barra de progreso
    },
        ...
        var updateProgressBar = function updateProgressBar(e) {
          ...
          if (slider.querySelector('.sladio__progressbar')) {
            var scrolled =
              ...
              100;
            slider.querySelector('#progressBar').style.width = ''.concat(
              scrolled,
              '%'
            );
    ...
    },
    {
      ...
      value: function fractionIndicator(slider, container, items, indicator) {
        ...
        var fractionText = document.createTextNode(
          '0 / '.concat(items.length - 1)
        );
        ...
        for (var i = 0; i < items.length; i++) {
          items[i].setAttribute(
            'data-index',
            ''.concat(i, ' / ').concat(items.length - 1)
          );
        }
    ...
    {
      key: 'bulletIndicator',
      value: function bulletIndicator(
        slider,
        container,
        items,
        indicator,
        ...
        for (var _i2 = 0; _i2 < items.length; _i2++) {
          if (_i2 % itemsToShow === 0)
            bulletsContainer.innerHTML +=
              '<a class="bullets" href="javascript:;" data-index='.concat(
                _i2,
                ' rel="nofollow"></a>'
              );
        ...
        (_allBullets$ = allBullets[0]) === null || _allBullets$ === void 0
          ? void 0
          : _allBullets$.classList.add('bullet_selected'); // Si interactive no esta activado, desactiva la función click
        ...
        var setSelected = function setSelected() {
          ...
          if (orientation === 'horizontal') {
            ...
            var nthChild =
              Math.round(container.scrollLeft / slider.scrollWidth) + 1 || 0; // Asignamos la clase dependiendo de la posición del scroll

            (_slider$querySelector = slider.querySelector(
              '.sladio__indicator__bullets a:nth-child('.concat(nthChild, ')')
            )) === null || _slider$querySelector === void 0
              ? void 0
              : _slider$querySelector.classList.add('bullet_selected');
          } // Si la orientación es vertical

          if (orientation === 'vertical') {
            // Escuchamos la posición del elemento
            var _nthChild =
              Math.round(container.scrollTop / slider.scrollHeight) + 1; // Asignamos la clase dependiendo de la posición del scroll

            slider
              .querySelector(
                '.sladio__indicator__bullets a:nth-child('.concat(
                  _nthChild,
                  ')'
                )
              )
    ...
    {
      key: 'autoChangeSlide',
      value: function autoChangeSlide(
        slider,
        container,
        items,
        interval,
    ...
    },
    {
      ...
      value: function responsiveSlides(
        items,
        desktop,
        tablet,
        mobile,
        ...
        if (desktop && window.innerWidth >= desktop.breakpoint) {
          ...
          for (var i = 0; i < items.length; i++) {
            items[i].style.minWidth = ''.concat(
              convertToPercentage.toFixed(2),
              '%'
            );
          } // if (itemsToShow !== 1) {
        ...
        if (
          ...
          for (var _i5 = 0; _i5 < items.length; _i5++) {
            items[_i5].style.minWidth = ''.concat(
              _convertToPercentage.toFixed(2),
              '%'
            );
          } // if (itemsToShow !== 1) {
          //   for (let i = 0; i < items.length; i++) {
        ...
        if (mobile && window.innerWidth <= mobile.breakpoint) {
          ...
          for (var _i6 = 0; _i6 < items.length; _i6++) {
            items[_i6].style.minWidth = ''.concat(
              _convertToPercentage2.toFixed(2),
              '%'
            );
          } // if (itemsToShow !== 1) {
          //   for (let i = 0; i < items.length; i++) {
    ...
    },
    {
      ...
      value: function report(type, message, color) {
        if (this._config.mode === 'development') {
          ...
          if (type === 'error')
            console.error('%c'.concat(this.message), 'color: '.concat(color));
          if (type === 'log')
            console.log('%c'.concat(this.message), 'color: '.concat(color));
          if (type === 'info')
            console.info('%c'.concat(this.message), 'color: '.concat(color));
        } // alert(this.message);
      },
    },
  ]);
